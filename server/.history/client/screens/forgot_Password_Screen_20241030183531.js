...
const ForgotPasswordScreen = ({ navigation }) => {
  ...
  const [step, setStep] = useState(1); // Thêm biến trạng thái để xác định bước hiện tại
  ...
  const validateOTP = (otp) => {
    const otpRegex = /^\d{6}$/; // Regex để kiểm tra OTP gồm đúng 6 chữ số
    return otpRegex.test(otp);
  };

  const validatePassword = (password) => {
    return password.length >= 6; // Ví dụ: Mật khẩu phải có ít nhất 6 ký tự
  };
